                ...
                <div class="flex flex-col">
                    <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                            ...
                            <form action="{{route('revisorreq.store')}}" method="POST">
                                @csrf 
                                ...
                                @foreach($autoridades as $autoridad)
                                    ...
                                    <div class="bg-emerald-100 shadow-md rounded px-2 pt-1 pb-2 mb-2 flex flex-col">
                                        ...
                                        <div class="-mx-3 md:flex mb-1">
                                            <div class="md:w-2/6 px-3 mb-1 md:mb-0">
        
                                                <input readonly onchange="habilitar()" required type="text" name="tipo{{$autoridad->tipo}}" id="tipo{{$autoridad->tipo}}"  value="{{$autoridad->tipo}}"
                                                      class=" text-sm w-full bg-emerald-50 text-black border border-lime-900 rounded py-1 px-3 mb-1">
                                                      <x-input-error :messages="$errors->get('tipo'.$autoridad->tipo)" />
                                            </div> 
                                            
                                                    <select onchange="habilitar()" required id="id_us{{$autoridad->tipo}}" name="id_us{{$autoridad->tipo}}" class="sel bg-emerald-50 border border-lime-900 text-black text-md py-2 px-4 pr-8 mb-2 rounded">
                                                     ...
                                                     <x-input-error :messages="$errors->get('id_us'.$autoridad->tipo)"/>
                                           <div class="md:w-1/6 px-3 mb-1 md:mb-0"></div>
                                        </div>
